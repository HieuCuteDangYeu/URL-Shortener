    ...
    IValidator<RefreshTokenRequest> refreshTokenValidator) : IAuthService
{
    ...
    public async Task<AuthResponse> RegisterAsync(RegisterRequest request)
    {
        ...
        try
        {
            ...
            var accessToken = jwtTokenGenerator.GenerateAccessToken(userId, user.Email, user.Role);
    ...
    public async Task<AuthResponse> LoginAsync(LoginRequest request)
    {
        ...
        try
        {
            ...
            var userWithCreds = await userServiceClient.GetUserByEmailAsync(getUserRequest);
            ...
            var accessToken = jwtTokenGenerator.GenerateAccessToken(userId, userWithCreds.Email, userWithCreds.Role);
            var refreshToken = jwtTokenGenerator.GenerateRefreshToken();
    ...
    public async Task<AuthResponse> RefreshTokenAsync(RefreshTokenRequest request)
    {
        ...
        // Generate new tokens
        var accessToken = jwtTokenGenerator.GenerateAccessToken(storedToken.UserId, user.Email, user.Role);
    ...
    public async Task RevokeTokenAsync(RevokeTokenRequest request)
    {
        var token = await dbContext.RefreshTokens
            .FirstOrDefaultAsync(rt => rt.Token == request.RefreshToken) ?? throw new InvalidOperationException("Token not found");
        if (token.IsRevoked)
